...
#include "../../misc/ImGuiColorTextEdit/TextEditor.h"
#include <sstream>
#include <fstream>
#include <iomanip>
#include <algorithm>
...
int main(int, char **)
{
    ...
    bool text_editor_initialized = false;
...
#endif
    {
        ...
        if (!ImGui::IsAnyItemActive() && !ImGui::IsAnyItemHovered() &&
            ...
            !ImGui::IsAnyMouseDown() && !show_text_editor)
        {
            SDL_Delay(45);
        }
        ...
        auto get_method_color = [&](int method) -> ImVec4
        {
            ...
            {
            case 0:
                return ImVec4(0.27f, 0.67f, 0.38f, 1.0f);
            case 1:
                return ImVec4(0.24f, 0.48f, 0.85f, 1.0f);
            case 2:
                return ImVec4(0.93f, 0.67f, 0.13f, 1.0f);
            case 3:
                return ImVec4(0.91f, 0.30f, 0.24f, 1.0f);
            case 4:
                return ImVec4(0.61f, 0.35f, 0.71f, 1.0f);
            case 5:
                return ImVec4(0.76f, 0.70f, 0.25f, 1.0f);
            default:
...

        {
            ...
            ImGui::Spacing();
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.44f, 0.37f, 0.61f, 1.0f));
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.54f, 0.47f, 0.71f, 1.0f));
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(0.34f, 0.27f, 0.51f, 1.0f));
            ...
            if (ImGui::BeginTabBar("request_tabs"))
            {
                ...
                if (ImGui::BeginTabItem("body"))
                {
                    ...
                    remaining_height -= ImGui::GetFrameHeightWithSpacing();
                    ImGui::InputTextMultiline("##body", &state.request_body, ImVec2(-1.0f, remaining_height));
...

        {
            ImGui::Begin("response body");
            {
                ...
                if (state.response_code >= 200 && state.response_code < 300)
                    status_color = ImVec4(0.27f, 1.00f, 0.38f, 1.0f);
                else if (state.response_code >= 300 && state.response_code < 400)
                    status_color = ImVec4(0.24f, 0.48f, 1.0f, 1.0f);
                else if (state.response_code >= 400 && state.response_code < 500)
                    status_color = ImVec4(0.98f, 0.67f, 0.13f, 1.0f);
                else if (state.response_code >= 500)
                    status_color = ImVec4(0.98f, 0.30f, 0.24f, 1.0f);
                else
        ...
        if (show_text_editor)
        {
            ...
            if (!text_editor_initialized)
            {
                ...
                textEditor.SetText("// start typing your code here...\n");
            ...
            if (ImGui::BeginMenuBar())
            {
                if (ImGui::BeginMenu("File"))
                {
                    ...
                    {
                        // implement file open if desired
                    }
                    if (ImGui::MenuItem("Save"))
                    {
                        // implement file save if desired
                    }
                    ImGui::EndMenu();
                }
                if (ImGui::BeginMenu("Edit"))
                {
                    ...
                    if (ImGui::MenuItem("Copy", "ctrl+c", nullptr, textEditor.HasSelection()))
                        textEditor.Copy();
                    if (ImGui::MenuItem("Cut", "ctrl+x", nullptr, !textEditor.IsReadOnly() && textEditor.HasSelection()))
                        textEditor.Cut();
                    if (ImGui::MenuItem("Paste", "ctrl+v", nullptr, !textEditor.IsReadOnly()))
                        textEditor.Paste();
                    ImGui::EndMenu();
                }
                if (ImGui::BeginMenu("View"))
                ...
                if (ImGui::Button("Prettify JSON"))
                {
                    std::string current_text = textEditor.GetText();
                    std::string formatted;
                    if (try_format_json(current_text, formatted))
                    {
                ...
                if (ImGui::IsItemHovered())
                {
                    ImGui::SetTooltip("Format JSON text");
                }
            }
